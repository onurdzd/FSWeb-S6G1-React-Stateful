...
export default function Programcilar() {
  ...
  const [oneCikanPromgramciId, setOneCikanProgramciId] = useState("");
  ...
  const oneCikaninIsmi = () => {
    ...
    return programcilar.find((item) => item.id === oneCikanPromgramciId).isim;
  };

  const stil = {
    fontSize: "1.5em",
    marginTop: "0.5em",
    color: oneCikanPromgramciId > 0 ? "gold" : "royalblue", // 🤔 kutlarken renk gold'a dönecek
  };
